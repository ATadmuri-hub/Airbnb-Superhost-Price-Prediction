...
# -----------------------------
# 1) Load exported matrices
# -----------------------------
x_train = pd.read_csv("x_kan_train.csv").to_numpy(dtype=np.float32)
y_train = pd.read_csv("y_kan_train.csv")["y"].to_numpy(dtype=np.float32).reshape(-1, 1)
x_test  = pd.read_csv("x_kan_test.csv").to_numpy(dtype=np.float32)
...
n_sub = min(6000, x_train.shape[0])
...
with torch.no_grad():
    ...
    probs_test = torch.sigmoid(logits_test).cpu().numpy().reshape(-1)
...
# -----------------------------
out = pd.DataFrame({"kan_pred_test": probs_test})
out.to_csv("kan_pred_test.csv", index=False)

print("Saved: kan_pred_test.csv")
print(out.head())
